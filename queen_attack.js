...
function queenCoordinates(board) {
  ...
  board.forEach((row, r) => {
    const wi = row.indexOf('W')
    if (wi !== -1) coordinates.w = [r, wi];
    const bi = row.indexOf('B')
    if (bi !== -1) coordinates.b = [r, bi]
  })
...
function attackableCoordinates(coordinate1, coordinate2) {
  ...
  const rowDiff = Math.abs(coordinate1[0] - coordinate2[0]);
  const colDiff = Math.abs(coordinate1[1] - coordinate2[1]);
...
function queenAttack(board) {
  ...
  const queens = queenCoordinates(parsedBoard)
  if (!('w' in queens) || !('b' in queens)) return undefined;
