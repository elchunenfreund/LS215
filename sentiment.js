...
function sentiment(text) {
  let positive = (text.match(positiveRegex) || []).map(w => w.toLowerCase());
  let negative = (text.match(negativeRegex) || []).map(w => w.toLowerCase());
  ...
  console.log(`There are ${positive.length} positive type words in the text.`);
  console.log(`Positive sentiments: ${positive.join(', ')}`);
  console.log('');
  console.log(`There are ${negative.length} negative type words in the text.`);
